 ...
 *
 * The `Beverage` class models a drink with various ingredients such as coffee
 * beans, cocoa powder, water, foam, and milk. Each ingredient is represented as
 * a normalized value between 0 and 1, where:
 * - **0** corresponds to the minimum amount of the ingredient.
 * - **1** corresponds to the maximum amount of the ingredient.
 *
 * The normalization is based on absolute minimum and maximum values, which are
 * defined for each ingredient. These minimun and maximum values are not
 * normalized and represent the actual limits for the ingredient in the
 * beverage.
 *
 * Each beverage has default normalized values for each ingredient, which define
 * the beverage. The ingredients amounts can be modified but the changes are
 * represented in the non-default ingredient attributes.
 *
 ...
 * - **max/min values**: Absolute maximum and minimum limits for each
 * ingredient.
 * - **defaultBeverageList**: A static list of default beverage instances.
 * Each ingredient has different limits and constants and the same for all
 * beverages(not normalized):
...
Q_PROPERTY(
    QString name READ name CONSTANT) // Map of ingredient names to amounts
    Q_PROPERTY(float coffeeBeans READ coffeeBeans WRITE setCoffeeBeans NOTIFY
                   coffeeBeansChanged) // Amount of coffee beans in the coffee
    Q_PROPERTY(float cocoaPowder READ cocoaPowder WRITE setCocoaPowder NOTIFY
                   cocoaPowderChanged) // Amount of cocoaPowder in the coffee
    Q_PROPERTY(float water READ water WRITE setWater NOTIFY
                   waterChanged) // Amount of water in the coffee
    Q_PROPERTY(float foam READ foam WRITE setFoam NOTIFY
                   foamChanged) // Amount of foam in the coffee
    Q_PROPERTY(float milk READ milk WRITE setMilk NOTIFY
                   milkChanged) // Amount of milk in the coffee

    Q_PROPERTY(float maxCoffeeBeans READ maxCoffeeBeans
                   CONSTANT) // Maximum amount of coffee beans in the coffee
    Q_PROPERTY(float maxCocoaPowder READ maxCocoaPowder
                   CONSTANT) // Maximum amount of cocoaPowder in the coffee
    Q_PROPERTY(float maxWater READ maxWater
                   CONSTANT) // Maximum amount of water in the coffee
    Q_PROPERTY(float maxFoam READ maxFoam
                   CONSTANT) // Maximum amount of foam in the coffee
    Q_PROPERTY(float maxMilk READ maxMilk
                   CONSTANT) // Maximum amount of milk in the coffee

    public
    : explicit Beverage(const QString &name, float coffeeBeans,
                        float cocoaPowder, float water, float foam, float milk,
