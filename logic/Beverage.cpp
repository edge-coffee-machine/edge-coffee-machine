...
// Constructor
Beverage::Beverage(const QString &name, float coffeeBeans, float cocoaPowder,
                   float water, float foam, float milk, QObject *parent)
...
float Beverage::coffeeBeans() const { return m_coffeeBeans; }
...
float Beverage::cocoaPowder() const { return m_cocoaPowder; }
...
float Beverage::water() const { return m_water; }
...
float Beverage::foam() const { return m_foam; }
...
float Beverage::milk() const { return m_milk; }
...
float Beverage::maxCoffeeBeans() const { return m_maxCoffeeBeans; }
float Beverage::maxCocoaPowder() const { return m_maxCocoaPowder; }
float Beverage::maxWater() const { return m_maxWater; }
float Beverage::maxFoam() const { return m_maxFoam; }
float Beverage::maxMilk() const { return m_maxMilk; }
float Beverage::getDefaultCoffeeBeans() const { return m_defaultCoffeeBeans; }
float Beverage::getDefaultCocoaPowder() const { return m_defaultCocoaPowder; }
float Beverage::getDefaultWater() const { return m_defaultWater; }
float Beverage::getDefaultFoam() const { return m_defaultFoam; }
float Beverage::getDefaultMilk() const { return m_defaultMilk; }
...
void Beverage::initDefaultBeveragesList() {
  ...
  Beverage::s_defaultBeverageList.append(
      new Beverage("Espresso", 0.2f, 0.4f, 0.3f, 0.5f, 0.6f));
  Beverage::s_defaultBeverageList.append(
      new Beverage("Cappuccino", 0.1f, 0, 0.3f, 0, 0.1f));
  Beverage::s_defaultBeverageList.append(
      new Beverage("Latte", 0.1f, 0, 0.3f, 0, 0.15f));
  Beverage::s_defaultBeverageList.append(
      new Beverage("Americano", 0.1f, 0, 1.0f, 0, 0));
  Beverage::s_defaultBeverageList.append(
      new Beverage("Mocha", 0.1f, 0.05f, 0.3f, 0, 0.12f));
  Beverage::s_defaultBeverageList.append(
      new Beverage("Milk", 0, 0, 0.2f, 0, 0));
  // Maybe we should notify here that the list has changed? but static methods
  // can't emit signals.
...
QList<Beverage *> Beverage::getIndependentBeverageList() {
  ...
  for (Beverage *bev : Beverage::s_defaultBeverageList) {
    Beverage *newBeverage = new Beverage(
        bev->name(), bev->getDefaultCoffeeBeans(), bev->getDefaultCocoaPowder(),
        bev->getDefaultWater(), bev->getDefaultFoam(), bev->getDefaultMilk());
