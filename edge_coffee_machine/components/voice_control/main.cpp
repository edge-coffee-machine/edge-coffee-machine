...
static void i2c_scan_existing_bus(i2c_master_bus_handle_t bus)
{
    const char *TAG = "I2C_SCAN";
    ESP_LOGI(TAG, "Scanning existing I2C bus...");
...
static void i2c_probe_es8311(i2c_master_bus_handle_t bus)
{
    const char *TAG = "I2C_PROBE";
    for (int addr : {0x18, 0x19}) {
        esp_err_t r = i2c_master_probe(bus, addr, pdMS_TO_TICKS(200));
        if (r == ESP_OK) {
            ESP_LOGI(TAG, "ES8311 ACK at 0x%02X", addr);
        } else {
            ESP_LOGE(TAG, "No ACK at 0x%02X (err=0x%x)", addr, (unsigned)r);
        }
    }
}

extern "C" void start_speech_recognition()
{
